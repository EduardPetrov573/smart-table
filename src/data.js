...
export function initData(sourceData) {
    ...
    const getIndexes = async () => {
        if (!sellers || !customers) {   // если индексы ещё не установлены, то делаем запросы
            [sellers, customers] = await Promise.all([   // запрашиваем и деструктурируем в объявленные ранее переменные
    ...
    const getRecords = async (query, isUpdated = false) => {
        ...
        if (lastQuery === nextQuery && !isUpdated) {    // isUpdated нужен, чтобы делать запрос без кеша
            return lastResult;                          // параметры не поменялись — отдаём сохранённые данные
        }

        // если прошлый квери не был установлен или поменялись параметры, то запрашиваем данные с сервера
